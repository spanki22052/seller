...
export function HomeTab({ onSubmit, isUpdating }: HomeTabProps) {
  const { t } = useTranslation();
  const [keywordsText, setKeywordsText] = React.useState("");

  const { data: seoPage, isLoading } = useQuery({
    queryKey: seoPagesKeys.detail("home"),
    queryFn: () => getSeoPageByType("home"),
  });
  ...
  if (isLoading) {
    return (
      <Card title={t("seo.homeTab", "Главная")}>
  ...
  return (
    <Card title={t("seo.homeTab", "Главная")}>
      <div style={{ marginBottom: 16 }}>
        <p>{t("seo.keywordsDescription", "Введите ключевые слова для главной страницы, каждое с новой строки:")}</p>
      </div>

      <Input.TextArea
        value={keywordsText}
        onChange={(e) => setKeywordsText(e.target.value)}
        placeholder={t("seo.keywordsPlaceholder", "купить читы\nлучшие читы для игр\nанти-чит система")}
        rows={10}
