...
import { useEditHomeLink } from "../hooks/useEditHomeLink";
import { UpdateHomeLinkDto, HomeLink } from "@/entities/home-link";
import * as Styled from "./styled";

interface EditHomeLinkFormProps {
  homeLink: HomeLink;
  onSuccess?: () => void;
}

export function EditHomeLinkForm({ homeLink, onSuccess }: EditHomeLinkFormProps) {
  const [form] = Form.useForm();
  const editHomeLink = useEditHomeLink();
  ...
  const handleSubmit = async (values: {
    ...
    try {
      ...
      await editHomeLink.mutateAsync({ id: homeLink.id, dto });
      onSuccess?.();
    } catch (error) {
  ...
  return (
    <Styled.FormWrapper>
      <Form
        ...
        disabled={editHomeLink.isPending}
      >
        <Form.Item
          name="title"
        ...
        <Form.Item>
          <Button
            type="primary"
            htmlType="submit"
            loading={editHomeLink.isPending}
            block
          >
            Обновить ссылку
          </Button>
